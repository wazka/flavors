...
class RandomBenchmark
{
  public:
	RandomBenchmark(
		...
		int deviceId,
		int maxMaskLength = 0,
		int minMaskLength = 0) : counts(counts),
	...
	RandomBenchmark(nlohmann::json &j, int maxMaskLength = 0, int minMaskLength = 0) : RandomBenchmark::RandomBenchmark(
...
class LengthBenchmark
{
  public:
	LengthBenchmark(
		...
		float minMaskLength = 0.0) : counts(counts),
									 randomCounts(randomCounts),
									 seeds(seeds),
	...
	LengthBenchmark(nlohmann::json &j, float maxMaskLength = 0.0, float minMaskLength = 0.0) : LengthBenchmark::LengthBenchmark(
																								   tryReadFromJson<std::vector<int>>(j, "counts"),
																								   tryReadFromJson<std::vector<int>>(j, "randomCounts"),
																								   tryReadFromJson<std::vector<int>>(j, "seeds"),
