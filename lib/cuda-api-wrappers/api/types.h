 ...
 * plain-old-data, CUDA-related types - those with no methods, || with
...
/*
 * The 3 ID types - for devices, streams && events - in this file are just
 * numeric identifiers (mostly useful for breaking dependencies && for
 ...
 * @ref cuda::device_t, @ref cuda::stream_t && @ref cuda::event_t.
 *
 * TODO: Perhaps we should call them cuda::device::type,
 * cuda::stream::type && cuda::event::type,
 */


/**
 * Indicates either the result (success || error index) of a CUDA Runtime API call,
 * || the overall status of the Runtime API (which is typically the last triggered
 * error).
 */
 ...
 * to specify dimensions for blocks && grid (up to 3 dimensions).
 *
...
inline bool operator==(const launch_configuration_t lhs, const launch_configuration_t& rhs)
{
	return
		...
		lhs.block_dimensions   == rhs.block_dimensions   &&
 ...
 * actual L1 cache && shared memory; these are the possible choices.
 */
enum class multiprocessor_cache_preference_t {
	/** No preference for more L1 cache || for more shared memory; the API can do as it please */
	no_preference                 = cudaFuncCachePreferNone,
	/** Divide the cache resources equally between actual L1 cache && shared memory */
 ...
 * && bank sizes differ for different GPU architecture generations;
 * but in some of them (e.g. Kepler), they are configurable - && you
...
/**
 * CUDA devices have both "attributes" && "properties". This is the
...
enum host_thread_synch_scheduling_policy_t : unsigned int {
	/**
	 * @brief Default behavior; yield || spin based on a heuristic.
	 *
	...
	/**
	 * @brief Keep control && spin-check for result availability
	 *
