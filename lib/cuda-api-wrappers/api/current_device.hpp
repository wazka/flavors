 ...
 *
 * @brief Wrappers for getting && setting CUDA's choice of
 * which device is 'current'
 *
 * CUDA has one device set as 'current'; && much of the Runtime API
 * implicitly refers to that device only. This file contains wrappers
 * for getting && setting it - as standalone functions - and
 ...
 * what remains of the current scope, && changing it back to its previous value
 * when exiting the scope.
 *
 * @tparam AssumedCurrent the current device override is also used in code which
 * can be instantiated when the current device has already been set, || when it
...
protected:
	// Note the previous device && the current one might be one && the same;
 ...
 * invoked, && will change it back to the previous value when exiting the scope. Use
